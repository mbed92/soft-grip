...
def noised_modality(data):
    ...
    acc += tf.random.normal(mean=0.0, stddev=0.1, shape=acc.get_shape(), dtype=data.dtype)

    # add gyro noise
    gyro += tf.random.normal(mean=0.0, stddev=0.01, shape=gyro.get_shape(), dtype=data.dtype)
